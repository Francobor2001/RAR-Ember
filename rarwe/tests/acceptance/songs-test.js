...
module('Acceptance | songs', function (hooks) {
  ...
  test('Sort song in various ways', async function (assert) {
    ...
    //# the query params has the title as 's'
    assert.ok(currentURL().includes('s=-title'),
      'The sort query params appears in the URL with the correct value'
    );
    ...
    debugger
    //# the query params has the rating as 's'
    assert.ok(
  ...
  test('Search songs', async function (assert) {
    ...
    this.server.create('song', { title: 'New Fang', rating: 3, band }); this.server.create('song', {
